"""CLI for the Lunar connector (issue #16).

Three subcommands map onto the Enable Banking AIS lifecycle:

* ``link``     — print the PSU consent URL for Lunar.
                 After consent the PSU is redirected to
                 ``--redirect-url`` with a ``?code=...`` query
                 parameter.
* ``authorize``— exchange that ``code`` for a ``session_id`` and
                 print the list of authorised account UIDs (incl.
                 each subaccount's product, which is how
                 Aktiesparekonto is detected).
* ``sync``     — pull booked transactions and the latest balance for
                 each account in the session and upsert into Postgres.
                 ASK subaccounts are auto-tagged with
                 ``dk_tax_treatment='aktiesparekonto'``.

The session_id is the unit of consent (~180 days for Lunar); save it
somewhere and pass via ``--session-id`` or ``LUNAR_SESSION_ID``.
"""
...
log = logging.getLogger("penge.ingest.lunar")
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="penge-lunar",
        description="Load Lunar transactions via Enable Banking PSD2 into Postgres.",
    )
    p.add_argument("-v", "--verbose", action="store_true")
    sub = p.add_subparsers(dest="command", required=True)

    pl = sub.add_parser("link", help="print PSU consent URL for Lunar")
    pl.add_argument(
        "--redirect-url",
        required=True,
    ...
    ps.add_argument(
        "--session-id",
        default=os.environ.get("LUNAR_SESSION_ID"),
        help="defaults to $LUNAR_SESSION_ID",
    )
    ps.add_argument(
        "--entity-name",
        default=os.environ.get("LUNAR_ENTITY_NAME", "Account Holder"),
        help="canonical entity (person) name; defaults to $LUNAR_ENTITY_NAME",
    )
    ps.add_argument(
        "--days",
        type=int,
...
def _cmd_authorize(args: argparse.Namespace, client: Client) -> int:
    ...
    sys.stderr.write(
        f"\nSave the session_id (export LUNAR_SESSION_ID={resp.session_id}) and run:\n"
        "  penge-lunar sync --session-id <SESSION_ID>\n"
    )
    return 0
...
def _cmd_sync(args: argparse.Namespace, client: Client) -> int:
    if not args.session_id:
        sys.stderr.write("error: --session-id (or $LUNAR_SESSION_ID) is required\n")
        return 2
    ...
    selected = [
        a
        for a in session.accounts_data
    ...
    for acct in selected:
        ...
        result = load_account(
            ...
            account_name=acct.name or acct.product or "Lunar account",
            currency=(acct.currency or "DKK").upper(),
