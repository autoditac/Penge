...
log = logging.getLogger("penge.ingest.ebank")
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="penge-ebank",
        description=("Load Evangelische Bank transactions via Enable Banking PSD2 into Postgres."),
    )
    p.add_argument("-v", "--verbose", action="store_true")
    sub = p.add_subparsers(dest="command", required=True)

    pl = sub.add_parser("link", help="print PSU consent URL for Evangelische Bank")
    pl.add_argument(
        "--redirect-url",
        required=True,
    ...
    ps.add_argument(
        "--session-id",
        default=os.environ.get("EBANK_SESSION_ID"),
        help="defaults to $EBANK_SESSION_ID",
    )
    ps.add_argument(
        "--entity-name",
        default=os.environ.get("EBANK_ENTITY_NAME", "Account Holder"),
        help="canonical entity (person) name; defaults to $EBANK_ENTITY_NAME",
    )
    ps.add_argument(
        "--days",
        type=int,
...
def _cmd_authorize(args: argparse.Namespace, client: Client) -> int:
    ...
    sys.stderr.write(
        f"\nSave the session_id (export EBANK_SESSION_ID={resp.session_id}) and run:\n"
        "  penge-ebank sync --session-id <SESSION_ID>\n"
    )
    return 0
...
def _cmd_sync(args: argparse.Namespace, client: Client) -> int:
    if not args.session_id:
        sys.stderr.write("error: --session-id (or $EBANK_SESSION_ID) is required\n")
        return 2
