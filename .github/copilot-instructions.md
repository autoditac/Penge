...
7. **Never merge a PR before the Copilot review bot has posted its review and every review thread is resolved.** "CI is green and no comments yet" is not approval — wait for the review to land. Never bypass branch protection with `gh pr merge --admin` or any equivalent. The user may explicitly override this for a single PR by recording the reason in the PR description; that override applies only to that PR.
