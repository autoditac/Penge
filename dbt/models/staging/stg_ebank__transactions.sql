...
-- Staging view for Evangelische Bank transactions.
--
-- Sourced from the Enable Banking PSD2 connector
-- (`penge.ingest.ebank`). The loader has already canonicalised kind
-- (CRDT→deposit, DBIT→withdrawal) and signed the amount, so this
-- view is a thin filter on `account.provider = 'ebank'`.
