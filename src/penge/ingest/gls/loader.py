"""GLS Bank → Postgres loader.

Wires :class:`penge.ingest.enablebanking.client.Client` into the
canonical schema. One ``load_account`` call:

1. fetches booked transactions and the latest balances for the
   account UID via the Enable Banking API,
2. ensures the operator's ``entity`` row, the GLS ``account`` row,
...
log = logging.getLogger("penge.ingest.gls.loader")
...
def _persist(
    ...
    *,
    ...
    log.info(
        "GLS load: account=%s booked=%d snapshots=%d",
        account_name,
        n_txn,
        n_snap,
    )
...
def _get_or_create_entity(conn: Connection, entity: Table, name: str) -> str:
    existing = conn.execute(
        select(entity.c.id)
        .where(entity.c.name == name, entity.c.kind == ENTITY_KIND)
        .limit(1)
    ).scalar_one_or_none()
