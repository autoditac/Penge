...
1. Sign up at <https://enablebanking.com/> and create an
   **Application**. For development you start in sandbox mode, which
   ...
   test against the real GLS until you upgrade to production
   credentials).
...
The session is valid for the consent duration (capped by GLS at
...
GLS (like Evangelische Bank and Lunar) returns booked balances with **no**
`reference_date` and no `last_change_date_time`. Because `/balances` reports
the account's *current* balance, the loader stamps such dateless balances with
the sync date (today, UTC) so the account still contributes to net worth
instead of showing €0. See [ADR-0043](../decisions/0043-enable-banking-dateless-balance-fallback.md).
...
banks. To do a true end-to-end test against GLS Bank you must
request production access from Enable Banking. The mock ASPSPs are
sufficient for verifying the consent dance, JWT signing, pagination,
and DB upserts.
